...
class Event implements \BoundedContext\Contracts\Sourced\Log\Event
{
    ...
    public function append_aggregate_events(Aggregate $aggregate)
    {
        ...
        $current_log_version = $this->log_version($state->aggregate_id(), $state->aggregate_type_id());
    ...
    private function log_version($aggregate_id, $aggregate_type_id)
    {
        $binary_aggregate_id = $this->binary_string_factory->uuid($aggregate_id);
        $binary_aggregate_type_id = $this->binary_string_factory->uuid($aggregate_type_id);
